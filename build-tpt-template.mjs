...
function buildCover() {
  ...
  const steps = [
    ...
    '11. Questions? Email: support@clearpatheg.com',
  ]
  ...
  // Footer
  setStr(ws, 30, 0, 'DAEP Student Tracker — Texas Edition  |  © 2025 Clear Path Education Group, LLC  |  clearpatheg.com')
...
function buildDashboard() {
  ...
  merge(ws, `A${r+1}:L${r+1}`)
  r++
  setStr(ws, r, 0, '© 2025 Clear Path Education Group, LLC  |  clearpatheg.com  |  For multi-campus and automated tracking, see the UPGRADE TO WAYPOINT tab.')
  merge(ws, `A${r+1}:L${r+1}`)
...
function buildReference() {
  ...
  // Footer
  rows.push(['© 2025 Clear Path Education Group, LLC  |  clearpatheg.com  |  This reference is for organizational use only — not legal advice.', '', '', ''])
...
function buildUpgrade() {
  ...
  rows.push(['Download our free "Texas DAEP Compliance Checklist" (PDF):  clearpatheg.com/resources', '', ''])
  rows.push(['Join our email list for Texas DAEP compliance updates:       clearpatheg.com/newsletter', '', ''])
...
function buildStatusBoard() {
  ...
  setStr(ws, r, 0, '─── Generated by DAEP Student Tracker — Texas Edition | Clear Path Education Group, LLC | clearpatheg.com ───')
