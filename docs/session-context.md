...
1. **Manual smoke walkthrough on `beacon.clearpathedgroup.com`** (NEW from CC11) — once Cloudflare deploy lands. Hard-refresh required (PWA service-worker cache). Test paths: Settings → Reminders & Calendar Export (enable browser reminders, send test, download .ics, import to Google Calendar); Sessions → Quick Log → Use template → fill 4-5 fields → save with the SOAP note populated; Dashboard first-load → Morning Brief card with stats + dismiss; Communications → log voicemail outcome → see follow-up appear, log 2 more → see escalation banner → Due-Process PDF; floating red 🚨 Crisis Now → walk through suicide workflow → confirm follow-ups + PDF.
