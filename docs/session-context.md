...
- **Migrations applied:** 001–064 (production). Migration 061: home campus DAEP handoff schema + trigger. Migration 062: `students.is_mtss`. Migration 063: students INSERT RLS for principal/ap/counselor. Migration 064: `navigator_student_monitors` (per-student configurable alerts). Home campuses seeded for Lone Star ISD (HS/MS/Elementary with DAEP seat allocations).
...
1. **TX-NDPA finalization** (NEW from CC10) — vendor-prep package drafted at `docs/legal/TX-NDPA-vendor-prep.md` + public `clearpath-site/dpa.html` page drafted, both held uncommitted. To unblock: user provides (a) Clear Path Education Group full registered legal address + state of formation, (b) authorized signatory name + title, (c) decision on `privacy@clearpathedgroup.com` mailbox vs `support@` for the privacy contact. Once those land: download TX-NDPA v1r6 PDF from SDPC, sign via DocuSign, post to vendor registry + commit the dpa.html page.
2. **Vera to rewrite Beacon user guide** — brief at `docs/handovers/vera-beacon-userguide-brief-04202026.md`. **OVERDUE** since 2026-04-25. Update needed: must now also cover the new Teacher Referral feature, Safety Alerts, District Preview, and CREST tracker shipped in CC10.
3. **CREST Phase 2** (NEW from CC10) — auto-populate suggested CREST artifacts from Beacon's existing data: Time Tracker → Cat-2 Use of Time Analysis; Lessons → Cat-5 Curriculum; Sessions/Referrals → Cat-4 Service Delivery. Plus portfolio PDF export + file attachments + District Preview rollup widget.
4. **Confirm first post-CC6 store purchase** — once the next real buyer comes through `store.html`, verify both browser-side notification + DB trigger fired, AND the Beacon/Apex welcome email link reaches them through a school firewall.
5. **District expansion readiness audit (Beacon)** — see CC9 entry: when a district shows up ready to sign a DPA, what's actually missing? Confirm: (a) Cloud auth + LoginPage flow wired end-to-end, (b) DPA template (now in progress via TX-NDPA above), (c) local→cloud migration tooling for individual counselors converted to district seats, (d) multi-counselor district admin UI, (e) seat-based license model in `product_licenses`. Inventory each gap with effort estimate.
6. **Test PDF templates visually** — generate sample PDFs from each product, verify page breaks work correctly after audit fixes. CC10 fixed Unicode gibberish in 3 District Preview reports — re-test all 6 reports + the legacy product PDFs to confirm.
7. **Toolkit B- → B+** — completeness indicator, contextual help, email case PDF.
8. **Consider publishing Waypoint pricing** with seat counts + term length — biggest remaining conversion lever to hit site grade A.
9. **Social proof** — get one named testimonial to move credibility score 6 → 8.
10. **Send demo emails** to 10 Formspree leads (carryover).
11. **Campus-scoped dashboard filter** — principals need campus dropdown.
12. **Cloud-mode Supabase migration for safety flags** (NEW from CC10) — local mode works immediately (IndexedDB schema-free), but cloud-mode districts using direct Supabase referral submission need `harm_to_self` / `harm_to_others` boolean columns added to the `referrals` table. Required before cloud-mode districts ship.
