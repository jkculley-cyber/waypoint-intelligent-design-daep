...
1. **Round-2 adversarial audit on Navigator (CC14 carryover)** — re-run the 3-voice cadence (Sam Ortega / Marsha Wilkerson / Reyes+Chen) against the post-T1+T2 Navigator. Beacon's pattern was 40 → 58 → 82 → 84 across rounds; Marsha's CC14 round-1 verdict was 42% so round 2 should land mid-50s to low 60s if the work is good. Convergent findings define T2.5/T2.6/T2.7 patches OR T3 promotions.
2. **5-minute UI walkthrough of Navigator T2 changes (CC14 carryover)** — Disproportionality by-race section (may need `students.race_ethnicity` populated in demo data first), Hearing Packet PDF on a student page (slate button), Escalation Engine duplicate detection in the bulk-create modal.
