...
export default function ApprovalChainTracker({ incidentId, onUpdate }) {
  ...
  const myPendingStep = steps.find(s => s.status === APPROVAL_STEP_STATUS.WAITING && userRole === s.step_role)
  ...
  return (
    <>
      <Card>
        {myPendingStep && (
          <div className="mb-4 -mt-1 p-3 bg-orange-50 border border-orange-200 rounded-lg flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-orange-800">Your Approval is Required</p>
              <p className="text-xs text-orange-600">This DAEP referral is waiting for your review as {myPendingStep.step_label}. Scroll down to approve, deny, or return.</p>
            </div>
          </div>
        )}
