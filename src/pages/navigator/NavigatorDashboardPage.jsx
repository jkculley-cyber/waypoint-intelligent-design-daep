...
export default function NavigatorDashboardPage() {
  ...
  const { monitors, dismissMonitor } = useStudentMonitors()
          ...
          <div className="bg-white rounded-xl border-2 border-amber-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-amber-100 bg-amber-50 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-amber-900">Student Monitoring Alerts</h2>
              </div>
              <span className="text-xs text-amber-700">{monitors.length} active</span>
            </div>
            <div className="divide-y divide-gray-50">
              {monitors.slice(0, 8).map(m => {
                const isOverdue = m.alert_date && new Date(m.alert_date) <= new Date()
                return (
                  <div key={m.id} className={`flex items-center justify-between px-5 py-3 ${isOverdue ? 'bg-red-50' : 'hover:bg-gray-50'}`}>
                    <Link to={`/navigator/students/${m.student_id}`} className="flex-1">
                      <p className="text-sm font-medium text-gray-900">
                        {m.students?.first_name} {m.students?.last_name}
                        <span className="ml-2 text-xs text-gray-500">Grade {m.students?.grade_level}</span>
                      </p>
                      <p className="text-xs text-gray-500 mt-0.5">
